  ...
  return (
    <>
      <div className="img">
        ...
        {props.gameDetail.metacritic && (
          <span
            className="metacitric-score"
            title="Metacritic Score"
            style={myStyle(props.gameDetail.metacritic)}
          >
            {props.gameDetail.metacritic}
          </span>
        )}
      ...
      <div className="title">
        ...
        {props.gameDetail.released && (
          <small className="date">{getDate(props.gameDetail.released)}</small>
        )}
        {props.gameDetail.parent_platforms?.map((el) => {
          return (
            <span key={el.platform.id} className="platform">
              {getPlatformsIcon(el.platform.slug)}
            </span>
          );
        })}
        <p className="my-3">
          Genre : {props.gameDetail.genres
                    ?.map((el) => {
                      return el.name;
                    })
                    .join(" , ")}
        ...
        <p>
          Platform : {props.gameDetail.platforms
                        ?.map((el) => {
        ...
        <p>
          Developers : {props.gameDetail.developers
                          ?.map((el) => {
                            return el.name;
                          })
                          .join(" , ")}
        ...
        <p>
          Publishers : {props.gameDetail.publishers
                          ?.map((el) => {
                            return el.name;
                          })
                          .join(" , ")}
        ...
        <p>
          Available On : {props.gameDetail.stores
                            ?.map((el) => {
        ...
        <button
          type="button"
          className="btn btn-outline-success btn-sm btn-style"
          id="library-btn"
          onClick={addToLibrary}
        >
          {currentUser && added 
          ? "In Library" 
          : (checkingLibrary ? <Miniloader/> : "Add To Library")}
        </button>
      </div>
    </>
  );
